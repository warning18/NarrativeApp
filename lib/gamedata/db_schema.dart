...
final DbSchema skillsSchema = DbSchema(
  ...
  fields: [
    ...
    FieldSchema(key: 'cost', label: 'Cost', type: FieldType.integer, defaultValue: 0),
    FieldSchema(key: 'requiredSkillID', label: 'Required Skill ID', type: FieldType.text),
...
final DbSchema questsSchema = DbSchema(
  ...
  fields: [
    ...
    FieldSchema(key: 'rewardItemID', label: 'Reward Item ID', type: FieldType.text),
    FieldSchema(
      key: 'alignmentChange',
    ...
    FieldSchema(key: 'nextQuestID', label: 'Next Quest ID', type: FieldType.text),
...
final DbSchema shopsSchema = DbSchema(
  ...
  fields: [
    ...
    FieldSchema(key: 'initialStock', label: 'Initial Stock (item IDs)', type: FieldType.stringList),
    FieldSchema(
      key: 'merchantSpecialties',
      label: 'Merchant Specialties (item types)',
...
final DbSchema gatesSchema = DbSchema(
  ...
  fields: [
    ...
    FieldSchema(
      ...
      label: 'Possible Monsters (enemy IDs)',
      type: FieldType.stringList,
    ),
  ],
);
...
final DbSchema adventureNodesSchema = DbSchema(
  ...
  fields: [
    ...
    FieldSchema(key: 'storyEventID', label: 'Story Event ID (Event)', type: FieldType.text),
    FieldSchema(key: 'shopID', label: 'Shop ID (Shop)', type: FieldType.text),
    FieldSchema(key: 'healPercent', label: 'Heal Percent (Rest)', type: FieldType.integer, defaultValue: 0),
    FieldSchema(
      key: 'guaranteedLoot',
      label: 'Guaranteed Loot (item IDs, Treasure)',
      type: FieldType.stringList,
    ),
    FieldSchema(
      key: 'goldReward',
